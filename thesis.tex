...
After each procedure it is assured that $L(\theta^{(p+1)};\mathbf{x}) \geq L(\theta^{(p)};\mathbf{x})$. Therefore, if we start from an initial estimate $\theta^{(0)}$ and iteratively compute $\theta^{(1)},\theta^{(2)},\theta^{(3)},\dots$, we will get a sequence of estimates, which converges to a local maximum.
...
\begin{itemize}
  ...
  \item The parameters of HMM can be estimated by the Baum-Welch algorithm, which is an application of the EM algorithm. There is no guarantee that the result is a global optimum.
\end{itemize}
