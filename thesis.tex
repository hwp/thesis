...
First of all, what HMM generates or measures, are discrete-time sequences of observed data. One observation is represented as $\mathbf{X}=\{X_t\}, t = 1, 2, \dots, T$. Here, $X_i$ can be from a finite set, $\{1, 2, \dots, K\}$, or a continuous space, $\mathbb{R}^d$. For simplicity, we will only consider the continuous case, since our system is exclusively used on continuous data.

Formally, an HMM consists of the following elements:
\begin{description}
  \item[The number of (hidden) states, $N$]
  \item[The state-transition probability, $A=\{a_{ij}\}$]
  \item[The obvervation probability distribution, $B=\{b_{i}(x)\}$]
  \item[The initial state distribution, $\pi=\{\pi_{i}\}$]
\end{description}
