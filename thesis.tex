...
Unlike the specific object recognition, generic category recognition deal with classifying an instance to a category. \Cref{fig:generic} shows an example of generic category recognition. For training, the system is given a set of objects, some of which belongs to a category (positive instances) and some of which does not (negative instances). For testing, the system need to tell whether an unseen object belongs to this category or not. The generic category recognition is more challenging since the objects belongs to the same category may vary in appearance.

\section{Thesis Outline}
The rest of the thesis will answer the aforementioned basic questions.
In \Cref{ch:visual,ch:audio}, we will present the visual and audio features for recognition systems.
In \Cref{ch:hmm}, we will introduce the theories of hidden Markov models.  
In \Cref{ch:bimodal}, we will describe the design of the bimodal object recognition system. Two modality fusion methods will be discussed.  
In \Cref{ch:experiment}, we will address how experiments are carried out and show the experimental results.  
Finally in \Cref{ch:conclusion}, the findings from the experiment will be discussed and suggested future work will be addressed.
...
In the following section, we will introduce the DoG detector and the SIFT descriptor, as these methods are used in the system.
...
\begin{itemize}
  ...
  \item The SIFT descriptor is invariant to rotation and illumination variation.
...
\label{ch:audio}
Audio signal processing is one of the most important fields of digital signal processing. Many different audio features have been proposed to represent the audio signal. In terms of good audio features, we want to find features such that, similar sounds that human perceive also appear similar (closer) in the feature space and the number of dimensions of the feature space should be small so that it can be applied to the subsequent learning algorithms. In other words, the feature should be simple and correspond to human perception.

Although audio features for object recognition have not been systematically studied, there has been extensive studies on audio features for speech recognition. The mel-frequency cepstrum coefficients (MFCCs) is one of the most commonly used audio features in speech recognition. Therefore, we choose MFCCs as the audio feature.
...
One shortcoming for STFT is that the frequency resolution and time resolution is mutually limited. This limit is known as the Gabor limit. The difference of two consecutive frequency bins is $\frac{f_s}{N}$. And, one STFT is a DFT of a neighborhood of $N$ samples, i.e. $N \times \frac{1}{f_s}$ time interval. That means the time resolution is $\frac{N}{f_s}$. Therefore, the product of the two resolutions, $\frac{f_s}{N} \times \frac{N}{f_s} = 1$, is constant. 

Selecting the windows size involves trade-off between the frequency resolution and time resolution. With a large window size, we can achieve good frequency resolution, while it can be difficult to catch the change in a short time. Vice versa, with a small windows size, we can achieve high time resolution. However, the frequency resolution would be low. \Cref{fig:spec} illustrates the spectrograms with different window sizes.
...
\begin{itemize}
  ...
  \item The mel-frequency cepstrum is an audio feature that encodes interpretation of sounds similar to human perception. It also captures information such as echo in the signal.
\end{itemize}
...
The increase of likelihood function can be proved as follows: for any $\mathbf{z}$, we know
\begin{equation}
...
Since $\lambda^{(p+1)}$ is a maximum of $Q(\lambda|\lambda^{(p)})$, we know $Q(\lambda^{(p+1)}|\lambda^{(p)}) - Q(\lambda^{(p)}|\lambda^{(p)}) \geq 0$. Therefore, $L(\lambda^{(p+1)};\mathbf{x}) \geq L(\lambda^{(p)};\mathbf{x})$.
...
\begin{itemize}
  ...
  \item The system incorporates two types of multimodal fusion methods. The feature fusion method is to learn HMM with concatenated features and directly estimate the joint likelihood. The decision fusion method is to learn two HMMs for individual modalities and compute the joint likelihood with the conditional independence assumption. The difference of the two methods is summarized in \Cref{tab:fusion}. Such two methods will be compared in the experiment.
\end{itemize}
...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{.8\textwidth}
    \centering
    \includegraphics[width=.25\textwidth]{knock0} \hspace{.4cm}
    \includegraphics[width=.25\textwidth]{knock1} \hspace{.4cm}
    \includegraphics[width=.25\textwidth]{knock2}

    \includegraphics[width=\textwidth]{knock.tikz}
    \caption{Strike}
  \end{subfigure}
  ...
  \begin{subfigure}[b]{.8\textwidth}
    \centering
    \includegraphics[width=.25\textwidth]{push0} \hspace{.4cm}
    \includegraphics[width=.25\textwidth]{push1} \hspace{.4cm}
    \includegraphics[width=.25\textwidth]{push2}

    \includegraphics[width=\textwidth]{push.tikz}
    \caption{Push}
  \end{subfigure}
  ...
  \begin{subfigure}[b]{.8\textwidth}
    \centering
    \includegraphics[width=.25\textwidth]{shake0} \hspace{.4cm}
    \includegraphics[width=.25\textwidth]{shake1} \hspace{.4cm}
    \includegraphics[width=.25\textwidth]{shake2}

    \includegraphics[width=\textwidth]{shake.tikz}
    \caption{Shake}
  \end{subfigure}
...
% There is some related study in this topic by using deep neural networks~\cite{ngiam_multimodal_2011}.
...
1 & \includegraphics[width=.12\linewidth]{object1} & No & No & No & No & Yes & No \tabularnewline 
2 & \includegraphics[width=.12\linewidth]{object2} & Yes & No & No & No & Yes & No \tabularnewline 
3 & \includegraphics[width=.12\linewidth]{object3} & Yes & No & No & No & Yes & No \tabularnewline 
4 & \includegraphics[width=.12\linewidth]{object4} & No & No & No & No & No & No \tabularnewline 
5 & \includegraphics[width=.12\linewidth]{object5} & No & No & No & No & No & No \tabularnewline 
6 & \includegraphics[width=.12\linewidth]{object6} & Yes & No & No & No & Yes & No \tabularnewline 
7 & \includegraphics[width=.12\linewidth]{object7} & No & No & No & No & Yes & No \tabularnewline 
8 & \includegraphics[width=.12\linewidth]{object9} & No & No & No & Yes & No & No \tabularnewline 
9 & \includegraphics[width=.12\linewidth]{object10} & No & No & No & Yes & No & No \tabularnewline 
10 & \includegraphics[width=.12\linewidth]{object11} & No & No & No & Yes & No & No \tabularnewline 
11 & \includegraphics[width=.12\linewidth]{object12} & No & No & No & Yes & No & No \tabularnewline 
12 & \includegraphics[width=.12\linewidth]{object13} & No & No & No & No & Yes & No \tabularnewline 
13 & \includegraphics[width=.12\linewidth]{object14} & No & No & No & Yes & No & No \tabularnewline 
14 & \includegraphics[width=.12\linewidth]{object16} & No & Yes & Yes & No & No & No \tabularnewline 
15 & \includegraphics[width=.12\linewidth]{object17} & No & No & Yes & No & No & Yes \tabularnewline 
16 & \includegraphics[width=.12\linewidth]{object18} & No & No & No & No & No & No \tabularnewline 
17 & \includegraphics[width=.12\linewidth]{object19} & No & Yes & No & No & Yes & Yes \tabularnewline 
18 & \includegraphics[width=.12\linewidth]{object20} & No & Yes & No & No & Yes & No \tabularnewline 
19 & \includegraphics[width=.12\linewidth]{object21} & No & Yes & No & No & Yes & No \tabularnewline 
20 & \includegraphics[width=.12\linewidth]{object22} & No & Yes & No & No & Yes & No \tabularnewline 
21 & \includegraphics[width=.12\linewidth]{object23} & Yes & No & No & No & Yes & No \tabularnewline 
22 & \includegraphics[width=.12\linewidth]{object24} & No & Yes & Yes & No & No & Yes \tabularnewline 
23 & \includegraphics[width=.12\linewidth]{object25} & No & No & No & No & Yes & No \tabularnewline 
24 & \includegraphics[width=.12\linewidth]{object26} & No & No & No & No & No & Yes \tabularnewline 
25 & \includegraphics[width=.12\linewidth]{object27} & No & No & No & No & Yes & No \tabularnewline 
26 & \includegraphics[width=.12\linewidth]{object28} & No & No & No & No & No & No \tabularnewline 
27 & \includegraphics[width=.12\linewidth]{object29} & No & No & Yes & No & No & Yes \tabularnewline 
28 & \includegraphics[width=.12\linewidth]{object30} & Yes & No & No & No & Yes & No \tabularnewline 
29 & \includegraphics[width=.12\linewidth]{object31} & No & No & Yes & No & No & No \tabularnewline 
30 & \includegraphics[width=.12\linewidth]{object32} & No & Yes & No & No & Yes & No \tabularnewline 
31 & \includegraphics[width=.12\linewidth]{object33} & No & No & Yes & No & No & No \tabularnewline 
32 & \includegraphics[width=.12\linewidth]{object34} & No & No & Yes & No & No & No \tabularnewline 
33 & \includegraphics[width=.12\linewidth]{object35} & No & No & Yes & No & No & Yes \tabularnewline 
\end{longtable}
