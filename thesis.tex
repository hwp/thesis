...
A process of parameters reestimation consisting of the following two steps:
\begin{description}
...
\begin{align}
  ...
  \log P(\mathbf{x}|\theta) &= Q(\theta|\theta') + H(\theta|\theta') \label{eq:loglph}.
\end{align}
...
\end{equation}
with equality if and only if $\theta = \theta'$. Therefore, together with Eq. \ref{eq:loglph},
\begin{align}
...
It is important to note that although the EM algorithm converges to a maximum estimate, there is no guaranty that the estimate is a global maximum (thus not a MLE). The resulting estimate is dependent on the initial estimate. Therefore, additional heuristic methods should be chosen for application of the EM algorithm.

\subsection{Baum-Welch Algorithm}
We have shown that the EM algorithm can be used for parameter estimation for models with latent variables and HMMs are examples or such models. Thus, applying the EM algorithm to the HMM would result in a solution for the third problem we stated in Section \ref{sec:hmm}. This method is known as the Baum-Welch algorithm.
...
\paragraph{E-step}
Consider parameters $\theta=(A,B,\pi)$ and $\theta'=(A',B',\pi')$, compute $Q(\theta|\theta')$:
\begin{align}
...
\begin{align}
  ...
  &= \sum_{i=1}^{N} \sum_{t=1}^{T} \gamma'(i) \log b_{i}(x_t).
\end{align}

\paragraph{M-step}
For fixed $\mathbf{x}$ and $\theta'$, to maximize $Q(\theta|\theta')$, we can see from Eq. \ref{eq:qhmmsep} that we only need to separately find parameters $\pi$, $A$ and $B$ that maximize $Q_\pi(\pi|\theta')$, $Q_A(A|\theta')$ and $Q_B(B|\theta')$.
