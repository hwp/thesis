...
\begin{figure}[t]
  ...
  \caption{The feature level fusion approach.}
  \label{fig:featuref}
\end{figure}

A direct approach is simply learn HMMs with the features concatenated, therefore the joint likelihood will be 
\begin{equation}
  P(\mathbf{v},\mathbf{a}|c) = P(\mathbf{v},\mathbf{a}|\lambda_{va}^c)
\end{equation}
in which $\lambda_{va}^c$ are the bimodal HMM parameters. This is an approach of feature level fusion. A block diagram of such approach is shown in \cref{fig:featuref}.
