...
\begin{align}
  \log P(\mathbf{x}|\theta) \sum_{\mathbf{z}} P(\mathbf{z}|\mathbf{x},\theta') 
    &= \sum_{\mathbf{z}} \log P(\mathbf{x},\mathbf{z}|\theta) P(\mathbf{z}|\mathbf{x},\theta')
    - \sum_{\mathbf{z}} \log P(\mathbf{z}|\mathbf{x},\theta) P(\mathbf{z}|\mathbf{x},\theta') \\
  \log P(\mathbf{x}|\theta) &= Q(\theta|\theta') + H(\theta|\theta') \label{eq:loglph}.
\end{align}
Here, $H(\theta|\theta')$ is defined as
\begin{equation}
  H(\theta|\theta') = - \sum_{\mathbf{z}} \log P(\mathbf{z}|\mathbf{x},\theta) P(\mathbf{z}|\mathbf{x},\theta')
\end{equation}
...
It is important to note that although the EM algorithm converges to a maximum estimate, there is no guaranty that the estimate is a global maximum (thus not a MLE). The resulting estimate is dependent on the initial estimate. Therefore, a good estimate should be computed with heuristics of the initial estimate.
