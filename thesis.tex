...
\section{Evaluation}
The performances are evaluated with two different criteria for the two different tasks. 
...
For the generic category recognition task, we evaluate the classification performance with object-based validation. Each time the video instances of one object is separated from the whole dataset to be the testing data and the rest are used for training. Therefore, all the testing objects are unseen objects, which are not in the training set.
...
\begin{table}
  \centering
  \begin{tabular}[h]{c|*{6}{b{.11\textwidth}}}
    \hline
    Method & Mugs & Bottles & Plastic Objects & Metal Objects & Fragile Objects & Containers with content \\ \hline \hline
    Feature Fusion & 0.750 & \textbf{0.802} & 0.820 & 0.830 & 0.711 & 0.671 \\
    Decision Fusion & \textbf{0.771} & 0.798 & \textbf{0.839} & 0.870 & 0.697 & \textbf{0.675} \\
    Visual Only & 0.763 & 0.778 & 0.813 & \textbf{0.877} & 0.590 & 0.567 \\
    Audio Only & 0.642 & 0.707 & 0.732 & 0.772 & \textbf{0.819} & 0.620 \\ \hline
  \end{tabular}
